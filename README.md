...
- Protected administration panel with dashboard
...
- SEO Foundation acceptance: `docs/SEO_FOUNDATION_ACCEPTANCE.md`
...
- `frontend/` - accepted production public frontend, protected Admin Dashboard, guarded candidate-application form integration, protected candidate/content administration, published News/Video integration, five predefined public managed-page routes and fail-safe SEO metadata/crawler behavior; public candidate submission remains intentionally disabled by default through `VITE_CANDIDATE_FORM_ENABLED=false`
...
- `infra/` - accepted Stage 5.1/5.2 Docker Compose runtime and security boundary, Stage 5.3A profile-only backup/restore operations for PostgreSQL and private media, Stage 5.3B finite email-worker/prune services plus host/systemd operations templates, Stage 5.3C dedicated `mail-egress` networking plus profile-only `smtp-check`, and Nginx crawler controls for `/admin`, `/api/` and `/healthz`
- `.github/workflows/ci.yml` - accepted read-only CI for pull requests to `main` and pushes to `main`: Backend uses Python 3.13 with Ruff and pytest; Frontend uses Node 22 with `npm ci`, typecheck, lint and production build; PostgreSQL Integration uses a temporary PostgreSQL 16.4 service, migrates a clean database to Alembic head and verifies the predefined managed-page seed invariants
...
The protected `/admin` route is now a real dashboard rather than a redirect to Candidates. It provides an exact-match `Обзор` navigation entry, four quick-access cards for Candidates/News/Video/Pages and up to five most recent candidate applications through the already accepted authenticated candidate-list API. No new backend endpoint, database aggregate or invented count was added. The first PR #52 CI run correctly caught a JSX syntax error through Frontend typecheck; after the targeted fix all three required CI jobs passed, and controlled local E2E visual review confirmed the dashboard, cards and protected recent-candidate rows in the existing `astrea-e2e` runtime.
...
SEO Foundation is accepted and documented in `docs/SEO_FOUNDATION_ACCEPTANCE.md`. Route metadata now has publication-aware `index/noindex` behavior, canonical links are emitted only when `VITE_PUBLIC_SITE_ORIGIN` is a valid bare non-local HTTPS origin, and the repository intentionally leaves that setting empty until the real production domain/subdomain is approved. `robots.txt` disallows `/admin`, `/api/` and `/healthz`, while Nginx adds `X-Robots-Tag: noindex, nofollow, noarchive` to those service paths. Controlled E2E verified the header on `/admin`, `/admin?probe=1`, `/api/v1/health` and `/healthz`, verified the public root remains indexable, verified localhost receives no canonical, and verified unpublished `/o-lozhe` is `noindex` with no canonical. Sitemap generation is deliberately deferred until the real production origin exists.
...
The GitHub repository default branch is `main`. CI PR #48 established the Backend and Frontend quality gates. CI PR #50 added the real PostgreSQL integration gate: PostgreSQL 16.4 became healthy, Alembic migrated an empty database through every revision to `20260824_0006`, the database revision matched the repository Alembic head, and the five accepted predefined pages were verified with exact titles/content and `is_published=false`. The same PR also passed Backend (`293 passed, 1 skipped`, Ruff) and Frontend (`npm ci`, typecheck, lint, build). An active `Protect main` ruleset now targets the default branch and requires `Backend`, `Frontend` and `PostgreSQL Integration`; GitHub reports `main` as protected. The baseline uses the ruleset mechanism rather than classic branch protection and does not add unrelated review, signed-commit or deployment requirements.

Current project direction explicitly freezes candidate legal activation and SMTP/mail work until client infrastructure/service-mailbox information is available. Production VPS/domain or subdomain, DNS/TLS, external secrets, backup/systemd provisioning, sitemap/production canonical activation and final cutover remain pending Stage 5.5 tasks.
