...
Current stage: Stage 4.4D1 persistent email outbox state machine accepted; Stage 4.4D2 SMTP delivery and worker execution is next.

The approved public design is documented in `docs/STAGE_2_DESIGN_FREEZE.md`. The production public frontend lives in `frontend/` and was accepted after visual and technical review. The backend foundation, public content domain, guarded candidate-intake pipeline, closed admin authentication, candidate administration API, content administration API and persistent email-outbox state machine live in `backend/`; protected candidate and content administration interfaces live in the production frontend.
...
Email delivery persistence now uses the existing `email_outbox` table with short committed claim transactions, PostgreSQL `FOR UPDATE SKIP LOCKED`, attempt-generation guards, bounded retry scheduling, stale-processing recovery and machine-safe failure codes. SMTP transport, email rendering and the executable worker loop remain deferred to Stage 4.4D2; the delivery model is intentionally at-least-once around SMTP rather than claiming impossible exactly-once delivery.
