...
Candidate intake is disabled by default. `CANDIDATE_INTAKE_ENABLED` must remain false until the approved privacy/consent documents, server-controlled legal version identifiers, public frontend wiring and deployment/security review are complete. Production scheduling/service wiring and real SMTP credentials/connectivity are deployment concerns and are not hidden inside the FastAPI process.
