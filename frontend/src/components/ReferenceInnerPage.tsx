  ...
  return (
    <ReferenceLayout>
      <div className="space-y-8">
        <header className="rounded-[6px] border border-brand-reference-line/35 bg-brand-reference-panel px-6 py-7 shadow-referenceCard lg:px-8 lg:py-9">
          {eyebrow ? (
            <p className="text-xs uppercase tracking-[0.14em] text-brand-reference-muted/55">{eyebrow}</p>
          ) : null}
          <h1 className="mt-2 font-referenceHeading text-[clamp(2rem,3.2vw,3.45rem)] font-normal leading-[1.04] text-brand-reference-text">
            {title}
          </h1>
          {lead ? (
            <>
...
export function ReferencePanel({ children, className = '' }: ReferencePanelProps) {
  return (
    <section
      className={`rounded-[6px] border border-brand-reference-line/30 bg-brand-reference-panel px-6 py-7 shadow-referenceCard lg:px-8 lg:py-8 ${className}`}
    >
      {children}
    </section>
  );
...
export function ReferenceNotice({ title, children, action }: ReferenceNoticeProps) {
  return (
    <ReferencePanel>
      <div className="border-l-2 border-brand-reference-red pl-5">
        <h2 className="font-referenceHeading text-2xl font-normal text-brand-reference-text">{title}</h2>
